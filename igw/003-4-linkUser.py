...
def lambda_handler(event, context):
    try:
        ...
        param = Param(event)
        print(param)
        partner_personal_detail = param.body.get("partnerPersonalDetail")
        ...
        # 查看设备状态的url
        if partner_personal_detail is not None:
            print("partner_personal_detail:--->", partner_personal_detail)
        url = "http://localhost:8080/user/platform/link"
        header = {
        ...
        # data = json.dumps({
        #     "platformUserId": platform_user_id,
        #     "partnerPersonalDetail": partner_personal_detail
        # })
        data = json.dumps({

            "partnerPersonalDetail": {
                "detailType": "mobile_phone",
                "detailValue": "13120575591"
            }
        })
        data = json.dumps({
